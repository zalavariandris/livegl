...
if __name__ == "__main__":
    ...
    # def on_event(event):
    #     if tool == "SELECTION":
    #         selectool.handle(event)

    #     if tool == "MOVE":
    #         movetool.handle(event)
    ...
    def on_input_event(event, *args, **kwargs):
        ...
        if tool == "SelectionTool":
            if event == "mousepress":
                mousepos = window.get_mouse_pos()
                selection_mask = np.zeros(model.controlpoints.shape[0], dtype=bool)
                selection_mask[model.selection] = True

                if any(under_mouse(mousepos, cp) for cp in model.controlpoints[selection_mask]):
                    tool = "MoveTool"
                else:
                    mousepos = window.get_mouse_pos()
                    P1 = mousepos[0], mousepos[1], 0
